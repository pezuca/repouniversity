...
var AlumnosAdmin = {
			...
			$.ajax({
				...
				success: function(data){
					$.gritter.add({
						...
						text: 'se añadio de forma exitosa el aulumno al grupo.',
						sticky: false
					});
		...
		editarTpAjax : function() {
			$.ajax({
				...
				success: function(data){
					...
					var celdas = $("#GruposAlumnos td a[data-tpgrupoId=" + data.id + "]").parents("tr").find("td");
		...
		deleteTpAjax : function(tpId) {
			$.ajax({
				...
				success: function(data){
					...
					table.row( $("#GruposAlumnos a[data-tpgrupoId=" + tpId + "]").parents('tr') ).remove().draw();
	...
	$(document).ready(function() {
		...
		$("#agregarAlumnoDialog").dialog({
			...
			buttons: {
				"Crear": function() {
					if(AlumnosAdmin.validacionFormlario("#nuevoAlumnoForm")) {
						AlumnosAdmin.crearNuevoAlumnoAjax();
					}
				},
				"Cancelar": function() {
					$(this).dialog("close");
		...
		$("#editarTpDialog").dialog({
			...
			buttons: {
				"Ok": function() {
					if(AlumnosAdmin.validacionFormlario("#editarTpForm")) {
						AlumnosAdmin.editarTpAjax();
					}
				},
				"Cancelar": function() {
					$(this).dialog("close");
		...
		$("#deleteTpDialog").dialog({
			...
			buttons: {
				"Eliminar": function() {
					AlumnosAdmin.deleteTpAjax($("#deleteTpDialog").data('tpId'));
				},
				"Cancelar": function() {
					$(this).dialog("close");
				}
