...
public class DashboardController {
	...
	public ModelAndView dashboard(HttpServletRequest request, @ModelAttribute("login") UsuarioRol usuario) {
		...
		if (usuario.getRol().equals("administrador"))
    	{
    		...
    		model.addObject("notificaciones", notificaciones).addObject("cursos", cursos).addObject("alumnos", alumnos).addObject("archivos", archivos).addObject("materias", materias).addObject("docentes", docentes).addObject("usuarios", usuarios).addObject("carreras", carreras);
	   	}
		return model;
	}
